...
class ExcelTable(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    table_name = db.Column(db.String(200), nullable=False, comment='表格名称')
    description = db.Column(db.Text, comment='表格描述')
    ...
    sheet_names = db.Column(db.Text, comment='工作表名称列表(JSON格式)')
    columns_info = db.Column(db.Text, comment='列信息(JSON格式)')
    ...
    def to_dict(self):
        return {
            ...
            'sheet_names': json.loads(self.sheet_names) if self.sheet_names else [],
...
class ExcelData(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    table_id = db.Column(db.Integer, db.ForeignKey('excel_tables.id'), nullable=False, comment='表格ID')
    sheet_name = db.Column(db.String(100), nullable=False, comment='工作表名称')
    row_index = db.Column(db.Integer, nullable=False, comment='行索引')
    column_index = db.Column(db.Integer, nullable=False, comment='列索引')
    cell_value = db.Column(db.Text, comment='单元格值')
    cell_type = db.Column(db.String(50), comment='单元格类型')
    created_at = db.Column(db.DateTime, default=datetime.utcnow, comment='创建时间')
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow, comment='更新时间')
    
    # 创建复合索引
    __table_args__ = (
        db.Index('idx_table_sheet_row_col', 'table_id', 'sheet_name', 'row_index', 'column_index'),
    )
...
@app.route('/smart_management')
def smart_management():
    """智慧管理模块"""
    return render_template('smart_management.html')

@app.route('/test_charts')
def test_charts():
    """智能图表测试页面"""
    return render_template('test_charts_frontend.html')

@app.route('/test_charts_detailed')
def test_charts_detailed():
    """智能图表详细测试页面"""
    return render_template('test_charts_detailed.html')

@app.route('/test_js')
def test_js():
    """JavaScript执行测试页面"""
    return render_template('test_js_execution.html')
...
@app.route('/api/refresh_project_summary', methods=['POST'])
def refresh_project_summary():
    """
    强制刷新项目总表
    
    用途：
    - 当源表数据发生变化时，手动触发项目总表重新生成
    - 确保项目总表数据与源表保持同步
    - 不影响源表数据，只更新项目总表
    """
    try:
        ...
        return jsonify({
            'success': False, 
            'message': f'刷新项目总表失败: {str(e)}'
        })


@app.route('/api/project_summary_legacy')
def get_project_summary_legacy():
    """获取项目总表（基于JOIN的传统方式）"""
    # 使用SQLAlchemy查询合并数据
...
# 删除重复的项目CRUD函数，使用下面更完整的版本

# 删除重复的资金安排CRUD函数，使用下面更完整的版本

# 删除重复的项目总表CRUD函数，项目总表是只读的，不需要CRUD操作

@app.route('/api/migrate_to_summary', methods=['POST'])
def migrate_to_summary():
    """将现有的项目信息和资金安排数据迁移到项目总表"""
    try:
        # 删除现有的总表数据
        ProjectSummary.query.delete()
        
        # 查询合并数据 - 使用LEFT JOIN显示所有资金安排项目
        query = db.session.query(
            FundingArrangement,
            ProjectInfo
        ).outerjoin(
        ...
        return jsonify({
            'success': True, 
            'message': f'数据迁移成功，共迁移 {migrated_count} 条记录到项目总表'
        })
    except Exception as e:
        db.session.rollback()
        return jsonify({'success': False, 'message': f'数据迁移失败: {str(e)}'})

# 删除未使用的图表数据API，图表功能在smart_charts页面中实现

# ==================== 智慧管理模块API ====================

@app.route('/api/excel_tables')
def get_excel_tables():
    """获取Excel表格列表"""
    try:
        tables = ExcelTable.query.order_by(ExcelTable.created_at.desc()).all()
        return jsonify({
            'success': True,
            'data': [table.to_dict() for table in tables]
        })
    except Exception as e:
        return jsonify({'success': False, 'message': f'获取表格列表失败: {str(e)}'})

@app.route('/api/excel_tables', methods=['POST'])
def upload_excel_table():
    """上传Excel表格"""
    try:
        ...
        # 获取表单数据
        table_name = request.form.get('table_name', file.filename.rsplit('.', 1)[0])
        ...
        for sheet_name in sheet_names:
            ...
            for row_idx, row in df_sheet.iterrows():
                for col_idx, value in enumerate(row):
                    if pd.notna(value):  # 只保存非空值
                        excel_data = ExcelData(
                            table_id=excel_table.id,
                            sheet_name=sheet_name,
                            row_index=row_idx,
                            column_index=col_idx,
                            cell_value=str(value),
                            cell_type=str(type(value).__name__)
                        )
        ...
        return jsonify({
            'success': True,
            'message': 'Excel表格上传成功',
            'data': excel_table.to_dict()
        })
        
    except Exception as e:
        db.session.rollback()
        return jsonify({'success': False, 'message': f'上传Excel表格失败: {str(e)}'})

@app.route('/api/excel_tables/<int:table_id>')
def get_excel_table_data(table_id):
    """获取Excel表格数据"""
    try:
        table = ExcelTable.query.get_or_404(table_id)
        sheet_name = request.args.get('sheet', table.sheet_names[0] if table.sheet_names else 'Sheet1')
        
        # 获取数据
        data_query = ExcelData.query.filter_by(
            table_id=table_id,
            sheet_name=sheet_name
        ).order_by(ExcelData.row_index, ExcelData.column_index)
        
        data_rows = data_query.all()
        
        # 转换为二维数组格式
        max_row = max([d.row_index for d in data_rows]) if data_rows else 0
        max_col = max([d.column_index for d in data_rows]) if data_rows else 0
        
        data_matrix = []
        for row_idx in range(max_row + 1):
            row_data = [''] * (max_col + 1)
            for col_idx in range(max_col + 1):
                cell_data = next((d for d in data_rows if d.row_index == row_idx and d.column_index == col_idx), None)
                if cell_data:
                    row_data[col_idx] = cell_data.cell_value
...
@app.route('/api/excel_tables/<int:table_id>/data', methods=['PUT'])
def update_excel_cell(table_id):
    """更新Excel单元格数据"""
    try:
        ...
        if row_index is None or column_index is None:
            return jsonify({'success': False, 'message': '缺少行索引或列索引'})
        
        # 查找或创建单元格数据
        cell_data = ExcelData.query.filter_by(
            table_id=table_id,
            sheet_name=sheet_name,
            row_index=row_index,
        ...
        if cell_data:
            if cell_value:
                cell_data.cell_value = str(cell_value)
                cell_data.updated_at = datetime.utcnow()
            else:
                # 如果值为空，删除记录
                db.session.delete(cell_data)
        else:
            if cell_value:  # 只有非空值才创建记录
                cell_data = ExcelData(
                    table_id=table_id,
                    sheet_name=sheet_name,
                    row_index=row_index,
        ...
        return jsonify({'success': True, 'message': '单元格更新成功'})
        
    except Exception as e:
        db.session.rollback()
        return jsonify({'success': False, 'message': f'更新单元格失败: {str(e)}'})

@app.route('/api/excel_tables/<int:table_id>', methods=['DELETE'])
def delete_excel_table(table_id):
    ...
    try:
        ...
        # 删除相关数据
        ExcelData.query.filter_by(table_id=table_id).delete()
        ...
        db.session.rollback()
        return jsonify({'success': False, 'message': f'删除表格失败: {str(e)}'})
...
def export_excel(table_type):
    ...
    try:
        ...
        # 创建Excel文件
        df = pd.DataFrame(data)
        print(f"调试信息 - 表格类型: {table_type}")
        print(f"调试信息 - 数据行数: {len(data)}")
        print(f"调试信息 - DataFrame列名: {list(df.columns)}")
    ...
    try:
        ...
        if not User.query.filter_by(username='admin').first():
            admin_user = User(
                ...
                password_hash=hash_password(os.getenv('DEFAULT_ADMIN_PASSWORD', 'changeme')),  # 默认密码，请修改
                real_name='系统管理员',
                department='信息技术部',
                position='系统管理员'
            )
...
def get_charts_data():
    ...
    try:
        # 直接从项目总表获取数据
        projects = ProjectSummary.query.all()
        ...
        for project in projects:
            chart_data.append({
                ...
                'arrangement_amount': float(project.arrangement_amount or 0),
                'funding_source': project.funding_source,
                'funding_nature': project.funding_nature,
                'budget_doc_no': project.budget_doc_no,
                'handler': project.handler,
                'handling_office': project.handling_office,
                'arrangement_year': project.arrangement_year,
                'superior_doc_no': project.superior_doc_no,
...
@app.route('/api/charts_analysis')
def get_charts_analysis():
    """获取图表分析数据 - 基于项目总表的多维度分析"""
    try:
        projects = ProjectSummary.query.all()
        ...
        status_distribution = {}
        for project in projects:
            status = project.project_status or '未知'
        ...
        type_distribution = {}
        for project in projects:
            project_type = project.project_type or '未知'
            type_distribution[project_type] = type_distribution.get(project_type, 0) + 1
        
        # 投资模式分布
        investment_distribution = {}
        for project in projects:
            investment_mode = project.investment_mode or '未知'
            investment_distribution[investment_mode] = investment_distribution.get(investment_mode, 0) + 1
        
        # 资金来源分布
        funding_source_distribution = {}
        for project in projects:
            funding_source = project.funding_source or '未知'
            funding_source_distribution[funding_source] = funding_source_distribution.get(funding_source, 0) + 1
        
        # 年度资金安排趋势
        yearly_funding = {}
        for project in projects:
            if project.arrangement_year:
                year = project.arrangement_year
                amount = float(project.arrangement_amount or 0)
                yearly_funding[year] = yearly_funding.get(year, 0) + amount
        ...
        for project in projects:
            region = project.region or '未知'
            amount = float(project.arrangement_amount or 0)
            region_investment[region] = region_investment.get(region, 0) + amount
        ...
        for project in projects:
            project_type = project.project_type or '未知'
            amount = float(project.arrangement_amount or 0)
            type_investment[project_type] = type_investment.get(project_type, 0) + amount
        ...
        return jsonify({
            ...
            'data': {
                ...
                'funding_source_distribution': funding_source_distribution,
                'yearly_funding': yearly_funding,
                'region_investment': region_investment,
                'type_investment': type_investment,
                'total_projects': len(projects),
                'total_funding': sum(float(p.arrangement_amount or 0) for p in projects),
...
@app.route('/api/pivot_table')
def get_pivot_table():
    """获取透视表数据 - 基于项目总表的多维度透视"""
    try:
        # 字段映射定义 - 与项目资金安排表筛选器字段保持一致
        # 使用OrderedDict保持字段顺序
        from collections import OrderedDict
        FIELD_MAPPING = OrderedDict([
            ('project_name', {'title': '项目名称', 'type': 'string'}),
            ('project_code', {'title': '项目编码（可研）', 'type': 'string'}),
            ('construction_unit', {'title': '建设单位', 'type': 'string'}),
            ('supervisor_dept', {'title': '项目主管部门', 'type': 'string'}),
            ('arrangement_amount', {'title': '安排金额(万元)', 'type': 'number'}),
            ('funding_source', {'title': '资金来源', 'type': 'string'}),
            ('funding_nature', {'title': '资金性质', 'type': 'string'}),
            ('budget_doc_no', {'title': '财预文号', 'type': 'string'}),
            ('superior_doc_no', {'title': '上级资金文号', 'type': 'string'}),
            ('arrangement_year', {'title': '安排年度', 'type': 'number'}),
            ('handler', {'title': '经办人', 'type': 'string'}),
            ('handling_office', {'title': '业务处', 'type': 'string'}),
            ('remarks', {'title': '备注', 'type': 'string'}),
            ('project_status', {'title': '项目状态', 'type': 'string'}),
            ('budget_amount', {'title': '概算批复金额(万元)', 'type': 'number'}),
            ('contract_amount', {'title': '签订合同金额(万元)', 'type': 'number'}),
            ('budget_code', {'title': '概算编码', 'type': 'string'}),
            ('budget_project_name', {'title': '概算项目名称', 'type': 'string'}),
            ('region', {'title': '所属区域', 'type': 'string'}),
            ('investment_mode', {'title': '投资模式', 'type': 'string'}),
            ('project_type', {'title': '项目类型', 'type': 'string'}),
            ('approval_date', {'title': '立项时间', 'type': 'date'}),
            ('start_date', {'title': '开工时间', 'type': 'date'}),
            ('end_date', {'title': '完工时间', 'type': 'date'})
        ])
        
        # 获取查询参数
        group_by = request.args.get('group_by', 'region')  # 分组字段
        value_field = request.args.get('value_field', 'arrangement_amount')  # 值字段
        agg_func = request.args.get('agg_func', 'sum')  # 聚合函数
        
        # 验证字段是否存在
        if group_by not in FIELD_MAPPING:
            return jsonify({'success': False, 'message': f'无效的分组字段: {group_by}'})
        if value_field not in FIELD_MAPPING:
            return jsonify({'success': False, 'message': f'无效的值字段: {value_field}'})
        ...
        for project in projects:
            # 获取分组值
            group_value = getattr(project, group_by, '未知') or '未知'
            
            # 获取数值
            value = getattr(project, value_field, 0) or 0
            value = float(value)
            
            if group_value not in pivot_data:
                pivot_data[group_value] = {
                    'count': 0,
                    'sum': 0,
                    'avg': 0,
                    'max': 0,
            ...
            pivot_data[group_value]['count'] += 1
            pivot_data[group_value]['sum'] += value
            pivot_data[group_value]['max'] = max(pivot_data[group_value]['max'], value)
            pivot_data[group_value]['min'] = min(pivot_data[group_value]['min'], value)
        
        # 计算平均值
        for group in pivot_data:
            if pivot_data[group]['count'] > 0:
                pivot_data[group]['avg'] = pivot_data[group]['sum'] / pivot_data[group]['count']
            if pivot_data[group]['min'] == float('inf'):
                pivot_data[group]['min'] = 0
        
        # 根据聚合函数选择结果
        result_data = []
        for group, stats in pivot_data.items():
            if agg_func == 'count':
                value = stats['count']
            elif agg_func == 'sum':
                value = stats['sum']
        ...
        return jsonify({
            ...
            'data': {
                ...
                'group_by': group_by,
                'group_by_title': FIELD_MAPPING[group_by]['title'],
                'value_field': value_field,
                'value_field_title': FIELD_MAPPING[value_field]['title'],
                'agg_func': agg_func,
        ...
        return jsonify({'success': False, 'message': f'获取透视表数据失败: {str(e)}'})
